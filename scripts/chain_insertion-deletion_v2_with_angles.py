...
"""
Description: 
This script deletes and inserts guest chains in a simulation box using Monte Carlo methods.

Algorithm:
    1. Extract the positions and bond data of the guest chains from the trajectory/config file.
    2. Determine the interface location (z-axis) on either side of the cavity.
    3. Iterate over every molecule in the "coords" array.
    4. If given molecule is a guest chain, then first determine its center of mass
    5. If COM is in the dilute regime, then we try to insert a chain in the cavity and if
       we are able to insert it, then delete this chain.
    6. Write a new config file that will be used as input for the next round of simulation.
    7. Save (append) the information regarding the inserted/deleted chains in another file. 
    

Input parameters:
    1. LAMMPS trajectory file
    2. LAMMPS config file
    3. TEXT file to save insertion/deletion information
"""
...
num_atoms, num_bonds, bond_data, num_angles, angle_data = utils.read_config(args.configuration)
bonds = np.array(bond_data) # (bond_id, bond_type, atom1, atom2)
angles = np.array(angle_data) # (angle_id, angle_type, atom1, atom2, atom3)
...
ax.plot(bin_centers_new, density_profile_host_new, marker="s", linestyle="", markerfacecolor="none")
...
fig.savefig(f"interface_location.pdf", dpi=300)
plt.close(fig)
...
print(f"Writing the updated configuration file: updated_config.dat \n")
utils.write_config("updated_config.dat",
                   simulation_box,
                   coords,
                   num_atoms,
                   num_bonds,
