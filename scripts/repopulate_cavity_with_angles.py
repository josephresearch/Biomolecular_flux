...
if final_nguest_cavity < args.target_num_guest_chains:
    ...
    for i in range(num_additional_chains):
        ...
        if len(new_chain_position) > 0:
            ...
            # Create a new angle data for the new chain
            new_angle_data = np.zeros((chain_length - 2, 5), dtype=int)
            new_angle_data[:,0] = np.arange(np.max(angles[:,0]) + 1, np.max(angles[:,0]) + 1 + chain_length - 2)
            new_angle_data[:,1] = 1 # angle type
            new_angle_data[:,2] = new_atom_ids[:-2]
            new_angle_data[:,3] = new_atom_ids[1:-1]
            new_angle_data[:,4] = new_atom_ids[2:]
            ...
            # Append the new angle data to the existing angle data
            angles = np.vstack((angles, new_angle_data))
